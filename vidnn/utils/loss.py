...
from vidnn.utils.ops import crop_mask, xywh2xyxy, xyxy2xywh
from vidnn.utils.tal import RotatedTaskAlignedAssigner, TaskAlignedAssigner, dist2bbox, dist2rbox, make_anchors, bbox2dist
from vidnn.utils.metrics import bbox_iou, probiou
...
class YoloDetectionLoss(nn.Module):
    """Criterion class for computing training losses for YOLO object detection."""

    def __init__(self, model, tal_topk: int = 10):  # model must be de-paralleled
        """Initialize YoloDetectionLoss with model parameters and task-aligned assignment settings."""
        super().__init__()
        h = model.cfg  # hyperparameters

        m = model.model.head  # Detect() module
        self.bce = nn.BCEWithLogitsLoss(reduction="none")
        self.hyp = h
        self.register_buffer("stride", m.stride)  # model strides
        self.nc = m.num_classes  # number of classes
        self.no = m.num_classes + m.reg_max * 4
    ...
    def __call__(self, preds: Any, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        imgsz = torch.tensor(feats[0].shape[2:], device=device, dtype=dtype) * self.stride[0]  # image size (h,w)
...
# class v8SegmentationLoss(YoloDetectionLoss):
...
# class v8PoseLoss(YoloDetectionLoss):
...
# class v8OBBLoss(YoloDetectionLoss):
...
#         self.one2many = YoloDetectionLoss(model, tal_topk=10)
#         self.one2one = YoloDetectionLoss(model, tal_topk=1)
...
#         self.vp_criterion = YoloDetectionLoss(model)
...
#         return [torch.cat((box, cls_vp), dim=1) for box, _, cls_vp in [xi.split((self.ori_reg_max * 4, self.ori_nc, vnc), dim=1) for xi in feats]]
