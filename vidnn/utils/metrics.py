...
from vidnn.utils import LOGGER, DataExportMixin, SimpleClass, TryExcept, plt_settings
...
def probiou(obb1: torch.Tensor, obb2: torch.Tensor, CIoU: bool = False, eps: float = 1e-7) -> torch.Tensor:
    ...
    t3 = (
        ((a1 + a2) * (b1 + b2) - (c1 + c2).pow(2)) / (4 * ((a1 * b1 - c1.pow(2)).clamp_(0) * (a2 * b2 - c2.pow(2)).clamp_(0)).sqrt() + eps) + eps
    ).log() * 0.5
...
def batch_probiou(obb1: Union[torch.Tensor, np.ndarray], obb2: Union[torch.Tensor, np.ndarray], eps: float = 1e-7) -> torch.Tensor:
    """
...
class ConfusionMatrix(DataExportMixin):
    ...
    def summary(self, normalize: bool = False, decimals: int = 5) -> List[Dict[str, float]]:
        ...
        return [
            dict({"Predicted": clean_names[i]}, **{clean_names[j]: array[i, j] for j in range(len(clean_names))}) for i in range(len(clean_names))
        ]
